...
#In case logger files are in another folder use:
...
#calculate Max Temp first

###### Calculate daily max temp
#Quick and dirty, can't do much with it:
DailyMax<- with(Water, tapply(Temp.C, Day, max))

#Better (can save as df for further use)
#ex:WaterMean<-aggregate(cbind(Temp.C, Light.Lux)~Day, Water, mean)
...
#Then calculate runing means,zoo pckg
rollmean(WaterDayMax$Temp.C, 7) #John asked for 7 days. Gives 38 values
#Add to df
